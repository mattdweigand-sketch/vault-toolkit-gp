...
## Purpose
Validate the proposed "why," then commit the record to the store and update the rolled-up patterns. This is where the loop closes — the per-run analysis becomes part of the workspace's memory. It is also the control gate: nothing enters the store as institutional memory without the deal lead / head of acquisitions signing off on the causal claims.

## Inputs
- **02_analysis/output/analysis-[deal]-[date].md**: The structured analysis with its stated-vs-assessed reason, proposed why, and confidence flags.
- **_config/store-schema.md**: The structure a stored record must follow.
- **_config/deal-taxonomy.md**: To confirm the tags used for aggregation, including the controlled broker list.
- **_store/**: The destination — records/ and patterns.md — and the current state of both.

## Process
1. Human validation first. The deal lead / head of acquisitions reviews the proposed "why," focusing on the claims the analysis flagged as low-confidence or pattern-contradicting — and especially anywhere the analysis is leaning on the broker's stated reason. Confirm, correct, or downgrade each causal claim. This step is not optional — it is the reason the store stays trustworthy and broker spin stays out of it.
2. Normalize the validated analysis into the store schema: the standard fields, the taxonomy tags, our bid vs. clearing price and the gap, the decisive dimension, the validated why with its final confidence, the stated-vs-assessed reason, the transferable lessons.
3. Write the record to `_store/records/[deal]-[date].md`. Do not overwrite prior records; the store is append-only history.
4. Update `_store/patterns.md`: does this record strengthen a pattern (add to its evidence), extend one (a new seller type, broker, or submarket), or contradict one (revise or qualify it)? Make the pattern change explicit and dated. A contradiction revises the pattern; it is not discarded to protect the existing story.
5. Note anything that should flow to another workspace now — a process type worth chasing for deal-pipeline sourcing, a bid-strategy adjustment, a screening-priority signal for deal-screening.
...
Validation: [causal claims confirmed / corrected — note any changes, especially
 where the broker's stated reason was downgraded in favor of an assessed one]
...
[Anything deal-pipeline sourcing / bid strategy or deal-screening should pick
 up now, or "none."]
```
...
- **Skipping validation.** Capturing the model's unvalidated "why" — or the broker's polite stated reason — turns the store into a pile of confident guesses that then drive real bids. The human sign-off is the control that makes the whole workspace worth trusting.

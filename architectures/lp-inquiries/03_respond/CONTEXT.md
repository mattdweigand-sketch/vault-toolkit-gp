...
## Purpose
Review the drafted response for accuracy and tone, confirm the figures trace to source and the requester is authorized, send it, log it, and capture any reusable answer back into the FAQ bank. This stage is the control gate before anything reaches an LP.
...
## Output
Write to: 03_respond/output/responded-[investor]-[date].md
...
## Done Looks Like
The LP has an accurate, on-voice response, or — for a sensitive inquiry — a clean acknowledgment while the right owner handles the substance. A record exists of what was sent, when, and that the figures tied to source. Reusable answers are in the FAQ bank.
...
- **Never feeding the FAQ bank.** If recurring answers are not captured, every IR person re-researches the same questions and the answers drift apart over time. Capture is what makes the workspace get faster and stay consistent.
